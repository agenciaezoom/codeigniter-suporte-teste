...
echo '[';
foreach ($files as $f){
  $fullPath = fixPath($path).'/'.$f;
  if(!is_file($fullPath) || ($type == 'image' && !RoxyFile::IsImage($f)) || ($type == 'flash' && !RoxyFile::IsFlash($f)) || $f == '.gitignore')
    continue;
  ...
  $str .= '{"p":"'.mb_ereg_replace('"', '\\"', $path.'/'.$f).'","s":"'.$size.'","t":"'.$time.'","w":"'.$w.'","h":"'.$h.'"},';
}
